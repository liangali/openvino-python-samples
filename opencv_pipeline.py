...
meta_data = []
frame_count, obj_count, obj_num = 0, 0, 0
encode_frame = 1
...
while True:
    ...
    for i in np.arange(0, detections.shape[2]):
        ...
        if confidence > threshold:
            ...
            #cv2.imwrite('./crop/roi_%02d.bmp' % (obj_num), crop_img)
            try:
                scale_img = cv2.resize(crop_img, (224, 224))
            except cv2.error as e:
                print('invalid crop', crop_img.shape)
...
meta_file = 'ref/opencv_meta_dc.txt'
with open(meta_file, 'wt') as f:
    f.writelines(meta_data)
    print('%d objects found in %d frames, ref meta data is written to %s' % (obj_num, frame_count, meta_file))
